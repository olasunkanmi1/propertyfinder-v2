...
const FurnishingStatus: React.FC<IChildProps> = ({handleDropdown}) => {
            ...
            return (
              <div key={placeholder} className='relative'>
                <div className='flex items-center justify-between bg-white rounded-md text-gray-600 py-2 px-4 text-md font-semibold cursor-pointer'
                    onClick={() => handleDropdown('furnishingStatus')}
                >
                    <p className='select-none'> Furnishing Status </p>
                    { dropdown ===  'furnishingStatus' ? <AiOutlineUp /> : <AiOutlineDown /> }

                </div>
                {dropdown === 'furnishingStatus' && <DropdownWithToggle state='purpose' title={placeholder} tabs={items} queryName={queryName} /> } 
              </div>
            )
