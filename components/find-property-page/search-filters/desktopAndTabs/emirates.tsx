...
const Emirates: React.FC<IChildProps> = ({handleDropdown}) => {
            ...
            return (
              <div key={placeholder} className='relative'>
                <div className='flex items-center justify-between bg-white rounded-md text-gray-600 py-2 px-4 text-md font-semibold cursor-pointer'
                    onClick={() => handleDropdown('emirates')}
                >
                    <p className='select-none'> Emirates </p>
                    { dropdown ===  'emirates' ? <AiOutlineUp /> : <AiOutlineDown /> }

                </div>
                {dropdown === 'emirates' && <DropdownWithToggle state='purpose' title={placeholder} tabs={items} queryName={queryName} /> } 
              </div>
            )
